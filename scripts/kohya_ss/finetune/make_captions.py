...
from extensions.sd_train_extensions.scripts.kohya_ss.finetune.blip.blip import blip_decoder
...
caption_weight = "models/BLIP/model_large_caption.pth"
print(f"loading BLIP caption: {caption_weight}")
model = blip_decoder(pretrained=caption_weight, image_size=IMAGE_SIZE, vit="large",
                     med_config="extensions/sd_train_extensions/scripts/kohya_ss/finetune/blip/med_config.json")
model.eval()
    ...
    def run_batch(path_imgs):
        ...
        for (image_path, _), caption in zip(path_imgs, captions):
            with open(os.path.splitext(image_path)[0] + args.caption_extension, "wt", encoding="utf-8") as f:
                f.write(post_process(caption) + "\n")
                if args.debug:
                    print(image_path, post_process(caption))
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--batch_size", type=int, default=10, help="batch size in inference / 推論時のバッチサイズ")
    parser.add_argument(
