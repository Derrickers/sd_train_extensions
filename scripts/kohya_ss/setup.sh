...
if [[ "$OSTYPE" == "lin"* ]]; then
  ...
  if [ "$RUNPOD" = true ]; then
    ...
    # else
    #   echo "${VENV_DIR}/lib/python3.10/site-packages/tensorrt/ not found; not linking extensions.sd_train_extensions.scripts.kohya_ss.library."
    # fi

    # if [ -d "${VENV_DIR}/lib/python3.10/site-packages/tensorrt/" ]; then
    #   export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:${VENV_DIR}/lib/python3.10/site-packages/nvidia/cuda_runtime/lib/"
    # else
    #   echo "${VENV_DIR}/lib/python3.10/site-packages/nvidia/cuda_runtime/lib/ not found; not linking extensions.sd_train_extensions.scripts.kohya_ss.library."
    # fi
